...
#{'criterion': 'entropy', 'max_depth': 10, 'n_estimators': 200} W1 & W2
## Multiclass Random Forrest Weights 1##
rf = RandomForestClassifier(criterion = 'entropy', max_depth= 10, n_estimators =200, class_weight=weights)
...
#{criterion = 'gini', max_depth = 5 , min_samples_split = 3 W1 (veel slechtere savings)
#{criterion = 'gini', max_depth = 3, min_samples_split = 2} W2
...
# Evaluating weighted Decision Tree Weights
cv = RepeatedStratifiedKFold(n_splits=5, n_repeats=2, random_state=42)
scores_dt = cross_val_score(dt, X, y, scoring="roc_auc_ovo", cv=cv, n_jobs=1)
print("roc_ovo Weighted DecisionTree (2) : %.3f" % np.mean(scores_dt))
...
## Multiclass LR Weights 1##
lr = LogisticRegression(multi_class='multinomial', penalty = 'l2', solver= 'newton-cg' ,class_weight=weights2
                        , max_iter=100)
lr.fit(X_tr_pp_m1, y_m1[ix_tr])
y_lr_pred = lr.predict(X_ts_pp_m1)
#multi_class='multinomial', penalty = 'l2', solver= 'newton-cg' ,class_weight=weights, max_iter=100 optimal
cv = RepeatedStratifiedKFold(n_splits=5, n_repeats=2, random_state=42)
...
Vclf = VotingClassifier(estimators=[('decisionTree', dt), ('RandomForest', rf),('LogisticRegression', lr)],
                        voting='soft')
